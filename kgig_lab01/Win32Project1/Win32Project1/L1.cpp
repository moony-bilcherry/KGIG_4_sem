...
void CMyMainWnd::Functions()
{
	...
	y_out = 30;
	x_out = 10;
	dc.TextOut(x_out + 70, y_out - 25, "Векторное произведение V1 * V2:");
	CMatrix Mult = VectorMult(*M.V1, *M.V2);
	PrintMatrix(dc, x_out, y_out, Mult);
	dc.TextOut(x_out + 70, (y_out+=120) - 25, "Скалярное произведение V1 * V2:");
	sprintf_s(t,"%3.2f",ScalarMult(*M.V1, *M.V2));
	dc.TextOut(x_out, y_out, t);

	dc.TextOut(x_out+70, (y_out +=60) - 25, "Модуль V1");
	sprintf_s(t, "%f", ModVec(*M.V1));
	dc.TextOut(x_out, y_out, t);

	dc.TextOut(x_out+70, (y_out += 60) - 25, "Модуль V2");
	sprintf_s(t, "%f", ModVec(*M.V2));
	dc.TextOut(x_out, y_out, t);

	dc.TextOut(x_out+70, (y_out += 60) - 25, "Косинус угла между V1 и V2:");
	sprintf_s(t, "%f", CosV1V2(*M.V1,*M.V2));
	dc.TextOut(x_out, y_out, t);

	dc.TextOut(x_out, (y_out += 60) - 25, "Сферические");
	dc.TextOut(x_out + 150, y_out - 25, "->");
	dc.TextOut(x_out + 200, y_out - 25, "Декартовы");
